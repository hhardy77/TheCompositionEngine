...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const regions = [...new Set(EXCERPTS.map(e => e.region))].sort();
  indexContainer.innerHTML =
    `<ul>` +
    regions.map(r => `<li><a href="regions.html?region=${encodeURIComponent(r)}">${r}</a></li>`).join("") +
    `</ul>`;

  function renderExcerpts(excerpts) {
    resultsContainer.innerHTML = "";
    if (!excerpts.length) {
      resultsContainer.innerHTML = "<p class='muted'>No excerpts for this region.</p>";
      return;
    }
