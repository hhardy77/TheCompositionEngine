...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const authors = [...new Set(EXCERPTS.map(e => e.author))].sort();
  indexContainer.innerHTML =
    `<ul>` +
    authors.map(a => `<li><a href="authors.html?author=${encodeURIComponent(a)}">${a}</a></li>`).join("") +
    `</ul>`;

  function renderExcerpts(excerpts) {
    resultsContainer.innerHTML = "";
    if (!excerpts.length) {
      resultsContainer.innerHTML = "<p class='muted'>No excerpts for this author.</p>";
      return;
    }
  ...
  if (author) {
    const filtered = EXCERPTS.filter(e =>
      e.author.toLowerCase() === author.toLowerCase()
    );
